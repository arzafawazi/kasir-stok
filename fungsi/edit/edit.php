...
if (!empty($_SESSION['admin']) || !empty($_SESSION['kasir'])) {
    ...
    $sql = 'UPDATE barang SET id_kategori=?, nama_barang=?, merk=?, harga_beli=?, harga_jual=?, satuan_barang=?, stok=?, tgl_update=? WHERE id_barang=?';

    $stmt = $config->prepare($sql);
    $stmt->execute($data);

    // Redirect to the previous page with success message
    header("Location: ../../index.php?page=barang&success-edit=edit-data");
    ...
    if (!empty($_GET['pelanggan'])) {
        ...
        $namapelanggan = htmlentities($_POST['namapelanggan']);
        $alamat = htmlentities($_POST['alamat']);
        $nomortelepon = htmlentities($_POST['nomortelepon']);
        
        $dataPelanggan[] = $namapelanggan;
        $dataPelanggan[] = $alamat;
        $dataPelanggan[] = $nomortelepon;
        $dataPelanggan[] = $id_pelanggan;
        
        $sqlPelanggan = 'UPDATE pelanggan SET namapelanggan=?, alamat=?, nomortelepon=? WHERE id_pelanggan=?';
        
        $rowPelanggan = $config->prepare($sqlPelanggan);
        $rowPelanggan->execute($dataPelanggan);
        
        echo '<script>window.location="../../index.php?page=pelanggan/edit&pelanggan='.$id_pelanggan.'&success=edit-data"</script>';
    }
